...
def analyze_strava_streams(file_path: str) -> Dict[str, Any]:
    ...
    analysis = {
        ...
        "recording_interval_seconds": time_data[1] - time_data[0]
        if len(time_data) > 1
        else 0,
        "frequency_hz": 1.0 / (time_data[1] - time_data[0])
        if len(time_data) > 1
        else 0,
        "available_streams": {},
    }
...
def print_analysis(analysis: Dict[str, Any]):
    ...
    print(
        f"Duration: {analysis['duration_minutes']:.1f} minutes ({analysis['duration_seconds']} seconds)"
    )
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("\n=== Key Findings ===")
        print(
            f"✅ Sensor data (power, cadence, HR) available at {analysis['frequency_hz']:.0f}Hz from Strava"
        )
        print(
            f"✅ {analysis['duration_minutes']:.0f} minute race = {analysis['total_points']:,} data points"
        )
        ...
        print(
            f"  • Sensor data: {analysis['frequency_hz']:.0f}Hz = 1 reading per second"
        )
        print("  • OCR extraction: ~4.8x faster in Rust (1.08s per frame)")
        print("  • Optimal strategy: Combine sensor feeds + OCR for complete telemetry")
