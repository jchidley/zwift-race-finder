...
def detect_all_text_regions(image_path: str, debug: bool = False) -> List[Dict]:
    ...
    for detection in result[0]:
        ...
        if debug:
            print(f"Detected: '{text}' at ({x_min}, {y_min}) size {region['width']}x{region['height']} conf={confidence:.2f}")
...
def classify_regions(regions: List[Dict], width: int, height: int) -> Dict[str, Dict]:
    ...
    for region in regions:
        ...
        elif x < width * 0.15 and height * 0.1 < y < height * 0.2 and text.replace('.', '').isdigit() and 'speed' not in classified:
...
def main():
    ...
    print("-" * 50)
    expected = [
        'speed', 'power', 'distance', 'altitude', 'race_time',
        'heart_rate', 'cadence', 'gradient', 'distance_to_finish', 'leaderboard'
    ]
    
    for field in expected:
        if field in classified:
            region = classified[field]
            print(f"✓ {field:<20} at ({region['x']:4}, {region['y']:4}) - '{region.get('text', 'N/A')}'")
        else:
