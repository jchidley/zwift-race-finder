...
class TelemetryStorage:
    ...
    def _init_csv(self):
        ...
        with open(self.csv_path, 'w', newline='') as f:
            writer = csv.DictWriter(f, fieldnames=[
                ...
                'race_time', 'power', 'cadence', 'heart_rate', 'avg_power',
                'energy', 'gradient'
            ])
            writer.writeheader()
    ...
    def add_frame(self, frame: TelemetryFrame):
        ...
        ''', (
            frame.timestamp, frame.frame_number, frame.speed, frame.distance,
            frame.altitude, frame.race_time, frame.power, frame.cadence,
            frame.heart_rate, frame.avg_power, frame.energy, frame.gradient
        ))
        self.conn.commit()
...
class ZwiftVideoProcessor:
    ...
    def process_frame(self, frame: np.ndarray, frame_number: int, timestamp: float) -> TelemetryFrame:
    ...
    def process_video(self, video_path: str, skip_frames: int = 30, 
                     realtime: bool = False, show_preview: bool = True) -> str:
        """
        ...
        print(f"  Processing every {skip_frames} frames ({fps/skip_frames:.1f} samples/sec)")
        ...
        try:
            while True:
                ...
                if frame_count % skip_frames == 0:
                    ...
                    try:
                        self.frame_queue.put((frame.copy(), frame_count, timestamp), timeout=0.1)
                        processed_count += 1
                    except queue.Full:
                ...
                try:
                    while True:
                        ...
                        if len(self.fps_counter) > 1:
                            actual_fps = len(self.fps_counter) / (self.fps_counter[-1] - self.fps_counter[0])
                            print(f"\rFrame {frame_count}/{total_frames} "
                                  f"({frame_count/total_frames*100:.1f}%) "
                                  f"Processing: {actual_fps:.1f} fps", end='')
    ...
    def process_stream(self, stream_url: str, duration: Optional[int] = None,
                      skip_frames: int = 30, show_preview: bool = True) -> str:
        """
        ...
        return self.process_video(stream_url, skip_frames, realtime=True, show_preview=show_preview)
...
def main():
    ...
    parser.add_argument('--realtime', action='store_true',
                       help='Process at video framerate')
    parser.add_argument('--analyze', action='store_true',
                       help='Analyze telemetry after extraction')
