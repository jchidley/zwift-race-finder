...
class RiderPoseDetector:
    ...
    def extract_features(self, avatar_region: np.ndarray) -> PoseFeatures:
        ...
        symmetry_score = 1 - (np.sum(np.abs(left_half - right_flipped)) / (min_width * edges.shape[0] * 255))
    ...
    def visualize_detection(self, avatar_image: np.ndarray, 
                          pose: RiderPose, 
        ...
        cv2.putText(result, f"Pose: {pose.value}", (10, 30),
        ...
        for i, text in enumerate(info_text):
            cv2.putText(result, text, (10, 60 + i * 20),
