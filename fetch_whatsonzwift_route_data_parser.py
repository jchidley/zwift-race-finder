        ...
        if tag == 'h4':
            ...
            if self.current_label and self.current_value:
                if self.current_label == "Distance":
                    ...
                    self.stats['lead_in_distance_km'] = self.parse_distance(self.current_value)
                elif self.current_label == "Lead-in elevation gain":
                    self.stats['lead_in_elevation_m'] = self.parse_elevation(self.current_value)
            self.in_row = False
...
def fetch_route_data(world, slug):
    ...
    try:
        ...
        if not parser.stats:
            print(f"  ✗ No route data found in HTML")
...
def main():
    ...
    parser.add_argument('--limit', type=int, default=10, help='Number of routes to process')
    parser.add_argument('--dry-run', action='store_true', help='Show what would be updated')
    parser.add_argument('--all-routes', action='store_true', help='Process all routes with slugs')
    ...
    try:
        ...
        print(f"WhatsOnZwift Route Data Parser")
        print(f"==============================")
        ...
        for i, (route_id, name, world, slug) in enumerate(routes):
            ...
            if data:
                ...
                print(f"    Lead-in elevation: {data.get('lead_in_elevation_m', 'N/A')} m")
