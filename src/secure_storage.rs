...
impl SecureStorage {
    ...
    fn load_from_env(&self) -> Result<StravaTokens> {
        Ok(StravaTokens {
            client_id: std::env::var("STRAVA_CLIENT_ID").context("STRAVA_CLIENT_ID not set")?,
    ...
    fn load_from_keyring(&self) -> Result<StravaTokens> {
        ...
        let json = entry.get_password().context("No tokens found in keyring")?;
        serde_json::from_str(&json).context("Failed to parse tokens from keyring")
    }
    ...
    fn load_from_file(&self, path: &Path) -> Result<StravaTokens> {
        ...
        serde_json::from_str(&contents).context("Failed to parse tokens from file")
    }
