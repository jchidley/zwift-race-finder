...
use crate::category::{get_category_from_score, get_detailed_category_from_score, get_category_speed};
use crate::event_analysis::find_user_subgroup;
use crate::parsing::{parse_description_data, estimate_distance_from_name};
...
fn log_unknown_route(event: &ZwiftEvent) {
    if let Ok(db) = Database::new() {
        if let Some(route_id) = event.route_id {
            ...
            let is_fixed_duration = event.duration_in_minutes.is_some() || event.duration_in_seconds.is_some();
            let event_type = if is_fixed_duration { "time" } else { "distance" };
...
fn display_route_based_duration(
    ...
    if let Some(route_data) = get_route_data(route_id) {
        let (actual_distance_km, lap_count) = calculate_actual_distance(
            &route_data, 
            user_subgroup, 
            distance_meters, 
            event
        ...
        display_unknown_route_duration(event, user_subgroup, distance_meters, route_id, zwift_score);
    }
}
