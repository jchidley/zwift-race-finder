...
pub fn database_connection_error(path: &str, err: &anyhow::Error) -> UserError {
    ...
    .with_suggestion(format!("Try creating the directory: mkdir -p $(dirname {})", path))
...
pub fn route_not_found(route_id: u32) -> UserError {
    ...
    .with_suggestion(format!("Report this route at: https://github.com/jchidley/zwift-race-finder/issues"))
}
...
pub fn zwiftpower_auth_error() -> UserError {
    ...
    .with_suggestion("You can get a new session ID by logging into zwiftpower.com and checking cookies")
}
