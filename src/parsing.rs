        ...
        for pattern in elevation_patterns {
            ...
            if let Some(caps) = re.captures(desc) {
                if let (Some(value), Some(unit)) = (caps.get(1), caps.get(2)) {
                    ...
                    data.elevation_m = Some(
                        if unit.as_str().contains("ft") || unit.as_str().contains("feet") {
...
mod tests {
    ...
    fn test_estimate_distance_from_name() {
        ...
        assert_eq!(
            estimate_distance_from_name("3R Flat Route Race"),
            Some(33.4)
        );
        assert_eq!(
            estimate_distance_from_name("Epic Pretzel Challenge"),
            Some(67.5)
        );
        assert_eq!(
            estimate_distance_from_name("Monday Night Crit Series"),
            Some(20.0)
        );
        assert_eq!(
            estimate_distance_from_name("Gran Fondo Saturday"),
            Some(92.6)
        );
        assert_eq!(
            estimate_distance_from_name("Century Ride Event"),
            Some(160.0)
        );
    ...
    fn test_parse_distance_from_name() {
        ...
        assert!(
            (miles_26_2 - 42.164708).abs() < 0.001,
            "Expected ~42.165, got {}",
            miles_26_2
        );
