// ABOUTME: Optimized OCR implementation using ocrs library
// Provides efficient text extraction matching ocrs CLI approach
...
/// Initialize and run OCR on an image using ocrs
pub fn extract_text(image_path: &Path) -> Result<String> {
    // Read the image and convert to RGB (matching ocrs CLI)
    let color_img = image::open(image_path)
        .map(|image| {
    ...
    // Prepare the image for OCR (matching ocrs CLI)
    ...
    // Get all text from the image as a single string
