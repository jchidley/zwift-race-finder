...
fn test_fetch_events_success() {
    ...
    let _m = server.mock("GET", "/api/public/events/upcoming")
        .with_status(200)
...
fn test_racing_score_event_parsing() {
    ...
    assert!(events[0]["description"].as_str().unwrap().contains("Distance: 35.1km"));
...
fn test_event_filtering_with_tags() {
    ...
    let ranked_events: Vec<_> = events.iter()
