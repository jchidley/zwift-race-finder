...
def parse_routes_ts():
    ...
    routes_match = re.search(r'export const routes.*?=.*?\(\s*\[(.*?)\]\s*\)', content, re.DOTALL)
    if not routes_match:
    ...
    for match in re.finditer(route_pattern, routes_content):
        ...
        lead_in_free = float(match.group(10)) if match.group(10) != 'undefined' else None
        lead_in_free_elev = int(match.group(11)) if match.group(11) != 'undefined' else None
        lead_in_meetup = float(match.group(12)) if match.group(12) != 'undefined' else None
        lead_in_meetup_elev = int(match.group(13)) if match.group(13) != 'undefined' else None
...
def update_database(routes):
    ...
    if 'lead_in_distance_km' not in columns:
        ...
        cursor.execute("ALTER TABLE routes ADD COLUMN lead_in_distance_km REAL DEFAULT 0.0")
        cursor.execute("ALTER TABLE routes ADD COLUMN lead_in_elevation_m INTEGER DEFAULT 0")
        cursor.execute("ALTER TABLE routes ADD COLUMN lead_in_distance_free_ride_km REAL")
        cursor.execute("ALTER TABLE routes ADD COLUMN lead_in_elevation_free_ride_m INTEGER")
        cursor.execute("ALTER TABLE routes ADD COLUMN lead_in_distance_meetups_km REAL")
        cursor.execute("ALTER TABLE routes ADD COLUMN lead_in_elevation_meetups_m INTEGER")
    ...
    for route in routes:
        ...
        cursor.execute("SELECT route_id FROM routes WHERE route_id = ?", (route['route_id'],))
...
def main():
    ...
    for route in routes[:5]:
        if route['lead_in_distance_km'] > 0:
            print(f"  - {route['name']}: {route['distance_km']} km + {route['lead_in_distance_km']} km lead-in")
